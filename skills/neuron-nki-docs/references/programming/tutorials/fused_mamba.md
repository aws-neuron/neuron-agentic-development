...
Figure below is a screenshot of the profile. We see this initial PyTorch implementation takes **151.83 ms** to execute _on
device_.
...
these two traits combined in the instruction sequence is considered extremely _inefficient_ on ScalarE/VectorE, where
...
on VectorE, which can perform the above loop nest in a _single_ instruction by caching the intermediate scan result from
...
a lower VectorE active time percentage is due to _blocking_ input tensor loading (`nl.load`) activities in the DMA.
...
**Loop-carried dependency.** Splitting `seq_len` into chunks is straightforward for all computation steps except for Step 4. In the associative scan operation, the next loop iteration requires results from the previous iteration for computation.
